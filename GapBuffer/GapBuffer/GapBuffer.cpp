...
GapBuffer::const_iterator GapBuffer::end() const {
	return { std::cbegin(data), std::cend(data), std::find(std::cbegin(data), std::cend(data), '\0'), const_cast<size_type*>(addressof(gap_start)), const_cast<size_type*>(addressof(gap_end)) };
}
